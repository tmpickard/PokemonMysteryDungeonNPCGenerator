...
namespace PokemonMysteryDungeonNPCGenerator.Models
{
    public class Pokemon
    {
        ...
        public Pokemon(int pokemonId, string species, byte[] image, int level, string gender, string type1, string type2, int hP, int attack, int defense, int sPAttack, int sPDefense, int speed, int overland, int sky, int swim, int levitate, int burrow, int highJump, int longJump, int power, int size, int weight, string acrobatics, string athletics, string charm, string combat, string command, string genEd, string medEd, string occEd, string pokeEd, string techEd, string focus, string guile, string intimidate, string intuition, string perception, string stealth, string survival, List<Move> movelist, List<Habitat> habitats, List<Capability> capabilities, List<Ability> abilities, List<Item> items)
        {
            PokemonId = pokemonId;
            Species = species;
            Image = image;
